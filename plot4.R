...
# create placeholders for 4 graphs in 2 columns and 2 rows
par(mfrow = c(2,2), mar = c(4,3, 2, 1))

with(df, {
     plot(DateTime, Global_active_power, type = "l", xlab = "", ylab = "Global Active Power (kilowatts")
     plot(DateTime, Voltage, type = "l", xlab = "datetime", ylab = "voltage")
     plot(x, Sub_metering_1, type = "l", col = "black", lwd = 0.25, ylim=c(0,40), xlab = '', ylab = "Energy sub metering")
     lines(x, Sub_metering_2, col = "red", lwd = 0.25)
     lines(x, Sub_metering_3, col = "blue", lwd = 0.25)
     legend("topright", legend = c("Sub metering 1", "Sub metering 2", "Sub metering 3"), lty = c(1,1,1), cex = 1, col = c('black', 'red', 'blue'))
     plot(DateTime, Global_reactive_power, type = "l", xlab = "datetime", ylab = "Global_active_power", ylim = c(0, 0.5))
